...
-export([init_fsm/1, init/2, prepare/2, abort/2, issue/2, transfer/2, cancel/2]).
...
%% Holder (Receive) that is defined in the TLA+ and DOT specifications.
%%
...
init(FsmPid, State) ->
    log("[~p] Doing init on holder (receive) in ~p state~n", [?MODULE, State]),
...
prepare(FsmPid, State) ->
    log("[~p] Doing prepare on holder (receive) in ~p state~n", [?MODULE, State]),
...
abort(FsmPid, State) ->
    log("[~p] Doing abort on holder (receive) in ~p state~n", [?MODULE, State]),
...
cancel(FsmPid, State) ->
    log("[~p] Doing cancel on holder (receive) in ~p state~n", [?MODULE, State]),
...
issue(FsmPid, State) ->
    log("[~p] Doing issue on holder (receive) in ~p state~n", [?MODULE, State]),
...
transfer(FsmPid, State) ->
    log("[~p] Doing transfer on holder (receive) in ~p state~n", [?MODULE, State]),
...
%% ---------------------------------------------------------------------
%% Valid states for a Holder (Receive) that is defined in the TLA+ and
%% DOT specifications.
%%
